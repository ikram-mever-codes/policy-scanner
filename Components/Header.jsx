...
const Header = () => {
  ...
  const handleClickOutside = (event) => {
    // Ensure the click is outside the dropdown and not on the links
    if (
      dropdownRef.current &&
  ...
  return (
    <AppBar
      ...
      color="transparent"
      elevation={0}
      className="top-0 px-4 sm:px-12 py-4 bg-white z-0 flex justify-between items-center"
    >
      <Toolbar className="w-full flex justify-between items-center">
        ...
        <nav className="hidden sm:flex items-center gap-8">
          ...
          <button className="w-max h-[2.8rem] flex justify-center items-center text-black bg-secondary text-nowrap px-[15px] py-[5px] gap-[10px] rounded-lg">
            <PhoneInTalkOutlinedIcon className="head-icon " />
            Schedule a Call
          </button>
        </nav>
